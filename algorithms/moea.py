...
class _GenCallback(Callback):
    ...
    def __init__(self, model, criterion, X_train, y_train, X_val, y_val,
                 weight_decay, logger, pbar):
        super().__init__()
        self.model = model
        self.criterion = criterion
        self.X_train = X_train
...
def run(
    ...
    print(f"Selected: train_loss={F[best_idx, 0]:.4f}, weight_norm²={F[best_idx, 1]:.4f}")